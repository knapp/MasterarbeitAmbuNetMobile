...
Struktur der Basisanwendung und Abschnitt \ref{sec:einbindenPlugins} erklärt Vorgang und Voraussetzungen, um Plug-Ins
zur Anwendung hinzuzufügen. Die Abschnitte \ref{sec:plugKontakte} und \ref{sec:plugTouren} stellen die beiden zentralen
Plug-Ins \textit{Kontakte} und \textit{Touren} vor.

\section{Aufgaben und Struktur der Basisanwendung}
\label{sec:basisanwendung}

Um einem Benutzer von \app die Bedienung der Anwendung zu erleichtern, sollte ein einheitlichesf Gesamterscheinungsbild
eingehalten werden, da ständige Strukturwechsel in der grafischen Oberfläche zu Unübersichtlichkeit führen können.
Hierzu wurde die in Abbildung \ref{fig:gui} gezeigte feste Struktur der grafischen Oberfläche entworfen. Oben links
befindet sich das Logo der Anwendung, darunter ist ein Bereich für die Darstellung von Informationen wie das aktuell
verwendete Plug-In oder der gerade behandelte Klient reserviert. Am oberen Rand, rechts des Logos, wurde eine Leiste zum
Platzieren von Buttons zur Navigation eingefügt. Hier sollen Schaltflächen zur Navigation zwischen Basisanwendung und
Plug-Ins platziert werden (z.B. ein "Home"-Button) und auch zur Navigation zwischen verschiedenen Sichten innerhalb
eines Plug-Ins. Der größte Bereich ist zur Darstellung und Bearbeitung des eigentlichen Inhalts gedacht.\\
Die gesamte Anwendung basiert lediglich auf zwei HTML-Dateien und einer Menge von JavaScript-Dateien. Eine HTML-Datei
wird für einen Login-Dialog verwendet, die zweite um den kompletten Inhalt darzustellen. Alle Strukturelemente und die
...
JavaScript entwickelt. Gegenüber der Entwicklung einer nativen Anwendung auf Basis einer objektorientierten
Programmiersprache wie Java (Android), Objective-C (iOS) oder C\#/C++ (Windows Phone) ergeben sich bei der Entwicklung
mit JavaScript gewisse Schwierigkeiten. Während bei einer objektorientierten Sprache Typsicherheit und Kapselung von
Funktionalität in Klassen zu den Eigenschaften gehören, ist JavaScript eine Scriptsprache, die keinerlei Typsicherheit
und nur bedingte Kapselungsmöglichkeiten bietet. Im Folgenden wird JavaScript stellvertretend für andere
objektorientierte Sprachen mit Java verglichen. Ein wichtiger Unterschied ist, dass Java vor der Ausführung zunächst
kompiliert, d.h. in eine maschinenverständliche Zwischensprache übersetzt werden muss, während JavaScript direkt vom
Browser interpretiert werden kann. Dies impliziert, dass der JavaScript-Code, selbst wenn er bereits auf einer Webseite
ausgeführt wird, dem Nutzer im Klartext zur Verfügung steht, während der Java-Code nur bedingt und durch aufwendige
Verfahren wieder dekompiliert und somit im Klartext verfügbar gemacht werden kann. Dieser Eigenschaft muss sich ein
Entwickler besonders im Bezug auf im Quellcode vermerkte sicherheitsrelevante Informationen bewusst sein.\\
...
werden. Eine Klassendefinition kann nur insofern erfolgen, dass einem Methodennamen der Name einer anderen vorher
...
verwendet, kann der Interpreter, der Browser, keine eindeutige Zuordnung des Namens zum ausführbarem Quellcode mehr
...
Laufvariablen in einer Schleife (häufig mit dem Namen \textit{i}), wird diese nun global geltende Variable alle anderen
...
unabdingbar. Zur Reduzierung der Wahrscheinlichkeit, dass \app unabsichtlich geschlossen wird, wurde der
...
Kommunikation. Zur Authentifizierung eines Nutzers wird Benutzername und Passwort als Zeichenkette der Form
\begin{center}\textit{Mandant/Benutzername:Passwort}\end{center}
an die Middleware übertragen. Die Verwendung von Benutzername und Passwort, welche auf Seiten des Servers auf
Richtigkeit überprüft werden, ist ein weit verbreiteter Authentifizierungsmechanismus, der auch \textit{Basic
authentication} genannt wird \cite[S. 204]{networkSecurity}. Um sicherzustellen, dass die
Authentifizierungsinformationen für CareNet lesbar sind, muss die Zeichenkette nach Base64 \cite{base64} kodiert werden.
Da PHP standardisierte Methoden zur Erstellung dieser Verschlüsselung anbietet, wurde entschieden, die Kodierung in die
Middleware zu verlagern.\\
Da eine Kodierung nach Base64 ein bekanntes Verfahren ist, wird hierdurch kein Schutzmechanismus für Benutzerdaten
implementiert. Diese werden praktisch im Klartext übertragen, da theoretisch jeder, der die Kommunikation abhört, die
Zeichenkette entschlüsseln kann. Aus diesem Grund muss die Verbindung an sich verschlüsselt werden. Hierzu wird eine
\acs{https}-Verbindung sowohl von \app zur Middleware als auch von der Middleware zu CareNet verwendet.\\
...
an den Empfänger übertragen. Dieser nutzt den Schlüssel, um eine Nachricht mit einem symmetrischen Schlüssel an den
Server zu schicken. Diesen symmetrischen Schlüssel können in der folgenden Kommunikation beide Seiten verwenden. Eine
langsamere asymmetrische Kommunikation findet also nur beim Aufbau der Verbindung statt \cite[S.
152]{sicherheitInternet}.
...
müssen den Namenskonventionen folgen, dass sie jeweils den Namen des Plug-Ins enthalten und ein beschreibendes Stichwort
...
\begin{figure}
	\centering
	\fbox{
		\includegraphics[scale=0.42]{figures/Startseite.jpg}
		}		
	\caption{Übersicht der verwendbaren Plug-Ins}
...
Tablet-PC geladen wurden. Hierzu wird eine lokale Sqlite-Datenbank von der Basisanwendung bereitgestellt. Die Einzelnen
...
bleiben die Daten verfügbar. Beim Start von \app wird eine Datenbank angelegt bzw. überprüft, ob bereits eine Datenbank
mit entsprechendem Namen vorhanden ist. Da die Datenbank innerhalb des WebViews angelegt wird und jede
PhoneGap-Anwendung einen eigenen anwendungsinternen WebView nutzt, kann nicht von anderen PhoneGap-Anwendung auf den
Speicher zugegriffen werden. So entsteht kein Sicherheitsproblem durch die Speicherung sensibler Daten auf dem
Tablet-PC. Alle Daten werden in Tabellen gespeichert, die in unbegrenzter Zahl erstellt werden können. Abbildung
\ref{fig:tablesContacts} zeigt die notwendige Tabellenstruktur, um Sichten und Kontakte lokal abspeichern zu können. Die
linke Tabelle enthält die \acs{id}s und Namen aller Sichten, die aus CareNet geladen wurden. In der rechten Tabelle sind
alle Kontakte mit \acs{id}s und einer Reihe von Attributen enthalten. Die mittlere Tabelle verbindet beide Tabellen,
indem sie durch Gegenüberstellung der \acs{id}s der Sichten und der Kontakte eine Abbildung schafft, welcher Kontakt in
welcher Sicht enthalten ist. Beispielsweise ist in Abbildung \ref{fig:tablesContacts} der Kontakt mit der \acs{id} C2 in
den Sichten mit den \acs{id}s V1 und V2 enthalten, während die anderen Kontakte jeweils nur in einer Sicht enthalten
sind.
...
einer Person angezeigt werden, jedoch wurden aus Gründen der Übersichtlichkeit diese Informationen gewählt.\\
...
Editierfunktion bestehender Kontakte oder die Neuanlage von Kontakten wurde nicht implementiert, da die Anforderung
nur die Anzeige von Kontaktinformationen vorsahen. Grundsätzlich ist eine Editierfunktion jedoch möglich.\\
...
von Intervallen hat den Effekt, dass Werte, die naturgemäß nicht möglich sind, wie z.B. ein Puls von 500 Schlägen pro
