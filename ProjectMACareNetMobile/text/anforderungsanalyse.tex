...
benötigen, um diese auf einem Tablet-PC abrufen und anzeigen zu können, wird VitaVBIT die Basis für das in Kapitel
\ref{ch:ziele} beschriebene zentrale System darstellen. Da es im Kontext dieser Arbeit aber nicht mehr
um die Dokumentation von Pflegetätigkeiten geht, sondern um die effiziente Übertragung ärztlicher Tätigkeiten an
Pflegepersonal, müssen Änderungen an VitaBIT vorgenommen werden. Um den Änderungen und dem Kontext der vernetzten Pflege
...
Interaktion dar. Im Gegensatz zu aufwendigen Sitzungen (Sessions), die von Servern während der Interaktion mit einem
Benutzer verwaltet werden müssen, werden bei dem REST-Prinzip vorberechnete Ressourcen, beispielsweise eine Webseite,
über eine \acs{uri} (\acl{uri}) referenziert und geladen. Alle Informationen über den Kontext einer Anfrage sind in der
...
Die Einführung einer Middleware hat zwei Gründe. Der erste ist die bereits in Kapitel \ref{ch:ziele} erwähnte
Platzhalterfunktion, um weitere Schnittstellen zu CareNet implementieren zu können, ohne CareNet selbst verändern zu
müssen. Der zweite Grund ist das so genannte \textit{Cross Domain Calling}-Problem
...
\begin{itemize}
  ...
  \item Erklärung, warum Middleware notwendig (Cross Domain Calling) und warum gewünscht (Erweiterung um
  ...
  \item Technologie des Software-Assistenten
  	\begin{itemize}
  	  ...
  	  \item Nativ vs. hybrid (Erklärung von PhoneGap und Zusammenhang mit jQuery mobile)
