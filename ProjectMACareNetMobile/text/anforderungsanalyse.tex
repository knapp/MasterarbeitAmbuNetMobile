...
Anwendung zur Dokumentation von übertragenen ärztlichen Tätigkeiten vorgestellt. In Abschnitt \ref{sec:analyse} werden
...
zu dokumentierenden Daten hervorgehen. Diese konnten auch nicht in der Literatur im Detail ermittelt werden, da einige
Leistungen medizinisches Fachwissen über deren Erbringung erfordern. Deswegen wurde zunächst angenommen, dass in der
Regel eine Bestätigung, ob eine Leistung erbracht wurde, ausreicht, und sonst nur die Dokumentation von Blutdruck und
Puls als wertbasierte Leistungen notwendig ist, sowie das Fotografieren von Wunden. Weitere zu dokumentierende Werte
konnten erst bei einem Treffen mit den Endnutzern ermittelt werden. Die Ergebnisse werden in Kapitel
...
sowohl stationär in einer Pflegeeinrichtung arbeiten, als auch im ambulanten Dienst tätig sein, bei der die
...
Bereich die gleichen sein werden, jedoch können sich Unterschiede bei den technischen Anforderungen und der
...
ausgeschlossen werden, dass Informationen über Personen verfügbar sind, die nichts mit der aktuellen Behandlung zu
tun haben.\\
...
Menge vordefinierter Leistungen erbracht werden muss. Zu Beginn jeder Tour bekommt eine Pflegekraft einen Plan
...
vollendeter Schicht manuell an das zentrale System übertragen und den richtigen Patienten zugeordnet werden müssen. Dies
...
automatische Erfassung würde Fehler, die bei der manuellen Eintragung von Messwerten auftreten können, ausschließen.
...
Dienste. Auch hier musste einer Pflegekraft ein Tourenplan angezeigt werden. Da die Leistungen ambulanter Dienste
...
an einem Bund zusammengefasst und mit Nummern zur Zuordnung versehen. Damit die Pflegekraft nicht erst lange nach dem
passenden Schlüssel suchen muss, war eine Anforderung, dass die Schlüsselnummer direkt in der Übersicht der Stationen
angezeigt wird. Im Falle der stationären Betreuung von Klienten soll anstatt einer Schlüsselnummer die Zimmernummer in
der Einrichtung angezeigt werden.\\
...
eines Klienten auf einer Karte anzeigen zu lassen. Alternativ könnte mit Hilfe des \acl{gps} (\acs{gps}) auch die
aktuelle Position relativ zur Adresse des Klienten angezeigt werden, um eine Routenplanung durchzuführen und somit
Fahrzeiten besser abschätzen zu können. Hierzu ist allerdings eine aktive Internetverbindung notwendig. Eine
vollständige Navigation ist nicht geplant.\\
...
Notwendigkeit der Verfügbarkeit eines Mobilfunknetzes, jedoch gilt diese Einschränkung ebenso beim Mitführen eines
...
\acs{rest} bedeutet eine lose Kopplung von vordefinierten Ressourcen, durch die ein Nutzer anhand gegebener
...
angestoßen, da in den vorangegangenen Projekten Probleme mit der verwendeten Hardware identifiziert wurden. Im
...
kleine Bildschirm die Möglichkeiten der Darstellung ein, was der Anforderung der Übersichtlichkeit entgegensteht.
...
Funktionalität, aber anderer Quellcodebasis erstellt werden. Dies bedeutet hohen zeitlichen Aufwand und ist
kostenintensiv.\\
...
Funktionalität. Mit reinen Webtechnologien wie HTML5 oder JavaScript ist es nämlich nicht möglich, hardwarespezifische
...
nativen Anwendung, einem nativen Container, einen Interpreter von Webtechnologien, WebView genannt, zu platzieren, der
Implementierungen in Webtechnologien wie HTML5, CSS3 und JavaScript interpretieren kann. Abbildung \ref{fig:phoneGap}
zeigt den Zusammenhang der verschiedenen Technologien. Um den Zugriff auf endgerätgspezifische Funktionen zu
ermöglichen, bietet PhoneGap zur Kommunikation mit allen wichtigen Hardware-Funktionen JavaScript-Methoden an, die auf
die jeweiligen Befehle in der nativen Programmiersprache abgebildet werden. Anstatt beispielsweise die Kamera unter
Android direkt über Java anzusprechen, erfolgt der Aufruf in JavaScript, welcher von einer Java-Klasse verarbeitet und
an das Betriebssystem weitergereicht wird. PhoneGap verfolgt somit einen hybriden Ansatz aus nativer Entwicklung und der
...
sowohl auf Android, als auch auf iOS oder Windows Phone bereitgestellt werden. Diese Möglichkeit bringt Vorteile in
Bezug auf Kosten und Zeit gegenüber der Entwicklung individueller nativer Anwendungen.\\
Eine Grundanforderung ist, dass über die mobile Anwendung Daten von einem Server abrufbar sein müssen. Würde nun
PhoneGap als Basis für die Anwendung gewählt, müsste die gesamte Funktionalität inklusive der Server-Kommunikation
mittels JavaScript implementiert werden. Ein Problem, welches hier auftreten könnte, ist ein Konflikt mit der von vielen
Browsern befolgten \textit{Same Origin Policy} (\cite{mozillaSop} und \cite{w3cSop}). Diese besagt, dass auf Seiten des
Internetnutzers ausgeführter Quellcode nur Aufrufe an die Adresse bzw. an den Server tätigen darf, von der die Dateien
geladen wurden, die den auszuführenden Quellcode (die Webseite) enthalten. Im Falle von PhoneGap werden die HTML- und
JavaScript-Dateien, die ausgeführt werden, von keinem Server geladen, sondern sie sind mit in die Anwendung eingebunden.
Nach der Same Origin Policy dürfte also nur an die Anwendung ein Aufruf mittels JavaScript erfolgen. Es wären also keine
Aufrufe an einen externen Server möglich und damit PhoneGap zur Kommunikation mit CareNet nicht in der Lage. Der
Hintergrund ist ein mögliches Sicherheitsproblem, wenn der Code, der eigentlich von einem vertrauenswürdigen Server
geladen wurde, lokal gespeicherte Daten ausliest (wie z.B. Zugangsdaten) und an eine andere nicht vertrauenswürdige Adresse
...
Verwendung von PhoneGap möglich ist, diese Option mit geringem Aufwand offen zu halten, wurde entschieden, \app als
...
In der Arbeit von Christina Hardt \cite{diplChristina} hat sich gezeigt, dass sich einmal gestellte Anforderungen rasch
ändern können und dass eine Anwendung so entworfen sein sollte, dass Anpassungen mit vertretbarem Aufwand durchgeführt werden können. Zwar ist
...
\begin{table}[ht]
	\begin{tabularx}{\textwidth}{|X|X|}
		...
		\hline		
		Übersichtlichkeit & Tablet-PC mit ausreichend großer Bildschirmfläche (Vorgabe)\\		
		\hline		
		Plattformunabhängigkeit, aber trotzdem Zugriff auf Hardeware-Funktionen (Kamera, GPS) & Entwicklung einer hybriden
		Anwendung auf Basis von PhoneGap\\
		\hline
		Touch-optimierte Benutzeroberfläche & Nutzung der JavaScript-Bibliotheken jQuery und jQuery mobile\\ 		
		\hline
		Flexibler Funktionsumfang, Erweiterbarkeit & Plug-In-Architektur\\
		\hline
		Erweiterbarkeit um Schnittstellen zu anderen zentralen System (außer CareNet) & Einführung einer Middleware, in der
		weitere Schnittstellen implementiert werden können (zunächst Platzhalterfunktion, da im Rahmen der Arbeit nur
		Anbindung an CareNet notwendig)\\
		\hline
		Möglichst wenige Einschränkungen bei der Entwicklung der Middleware, Portierbarkeit auf verschiedene Server &
		Nutzung von PHP zur Implementierung der Middleware (weit verbreitet und nahezu auf jedem Server lauffähig)\\
		\hline
	\end{tabularx}
